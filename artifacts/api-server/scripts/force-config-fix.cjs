...
// Since I can't easily require TS, I'll just copy the DEFAULT_SECTIONS here for the fix
const SECTIONS = [
  {
    id: "instructions",
...
async function fix() {
  try {
    console.log("Forcing configuration for all forms...");
    const jsonStr = JSON.stringify(SECTIONS);
    await pool.query("UPDATE application_forms SET sections_config = $1", [jsonStr]);
    console.log("Updated all forms with standard sections.");
    
    // Also ensure the July 2026 program exists and has a form
    ...
    // Delete any forms that are NOT "July 2026"
    await pool.query("DELETE FROM application_forms WHERE title NOT LIKE '%July 2026%'");
    console.log("Deleted non-July 2026 forms.");
